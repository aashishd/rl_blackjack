...
if __name__ == '__main__':
    ...
    for gamma in [0.7, 0.8, 0.95, 0.99]:
        filepath = f'pi_stats/Blackjack-PI-gamma_{gamma}.pickle'
        mdp, policy_mdp, vi_stats = run_policy_iteration(T, R, GAME_NAME, gamma=gamma, return_mdp=True)
        print(f'\n*********************Stats for gamma = {gamma} *********************\n')
        # policy_grid, values_grid = get_policy_grid(policy_mdp, mdp.V)
        # fig1 = create_plots(values_grid, policy_grid, 'Blackjack without usable ace')
        # plt.show()
        pickle_mdp_model(filepath, mdp)

        policy_grid, values_grid = get_policy_grid(policy_mdp, mdp.V)
        fig1 = create_plots(values_grid, policy_grid, rf'PI Policy for Blackjack without usable Ace $\gamma$={gamma}')
        plt.show()

        policy_grid, values_grid = get_policy_grid(policy_mdp, mdp.V, usable_ace=True)
        fig2 = create_plots(values_grid, policy_grid, rf'PI Policy for Blackjack with usable Ace $\gamma$={gamma}')
        plt.show()
